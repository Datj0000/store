...
class ImportDetailController extends Controller
{
    ...
    public function fetchdata(int $id)
    {
        if (Auth::check()) {
            ...
            $output = '
            <input type="hidden" id="importdetail_server_id" value="'.$server_id.'"/>
            <div class="card-body">
                ...
                foreach ($data as $key => $item) {
                    ...
                    $output .= '
                        <tr>
                            ...
                            $output .='
                            <td><input id="edit_importdetail_name_'.$item->id.'" class="custom_input" value="'.$item->importdetail_name.'" type="text"></td>
                            <td><input id="edit_importdetail_link_'.$item->id.'" class="custom_input" value="'.$item->importdetail_link.'" type="text"></td>
                            <td>';
                            if ($item->importdetail_status == 0) {
                                $output .='<span data-id_importdetail='.$item->id.' class="unactive_importdetail btn btn-sm btn-clean btn-icon" title="Đang hiển thị">
                                    <i class="la la-eye"></i>
                                </span>';
                            } else {
                                $output .='<span data-id_importdetail='.$item->id.' class="active_importdetail btn btn-sm btn-clean btn-icon" title="Đang ẩn">
                            ...
                            <td>
                                ...
                                <span data-id='.$item->id.' class="delete_importdetail btn btn-sm btn-clean btn-icon" title="Xoá">
