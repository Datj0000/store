                ...
                foreach ($data as $key => $item) {
                    ...
                    $output .= '
                        <tr>
                            ...
                            <td style="with: 15%"><input id="edit_episode_name_'.$item->episode_id.'" class="custom_input" value="'.$item->episode_name.'" type="text"></td>
                            <td style="with: 45%"><input id="edit_episode_link_'.$item->episode_id.'" class="custom_input" value="'.$item->episode_link.'" type="text"></td>
                            <td style="with: 15%">';
                            if ($item->episode_status == 0) {
                                $output .='<span data-id_episode='.$item->episode_id.' class="unactive_episode btn btn-sm btn-clean btn-icon" title="Đang hiển thị">
                                    <i class="la la-eye"></i>
                                </span>';
                            } else {
                                $output .='<span data-id_episode='.$item->episode_id.' class="active_episode btn btn-sm btn-clean btn-icon" title="Đang ẩn">
                                    <i class="la la-eye-slash"></i>
                                </span>';
                            }
                            $output .='</td>
                            <td style="with: 15%">
                                <span data-id_episode='.$item->episode_id.' class="edit_episode btn btn-sm btn-clean btn-icon" title="Sửa">
                                    <i class="la la-edit"></i>
                                </span>
                                <span data-id_episode='.$item->episode_id.' class="delete_episode btn btn-sm btn-clean btn-icon" title="Xoá">
