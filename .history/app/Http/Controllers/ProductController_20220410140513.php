                ...
                foreach ($product as $key => $val){
                    ...
                    if($detail){
                        if($detail->detail_quantity - $detail->detail_soldout > 0){
                            if(Auth::user()->role <= 1){
                                $output .= '<li class="li_search_product" data-code="'.$val->product_code.'">' . $val->product_code . ' - ' . $val->brand_name . '  ' . $val->product_name . ' - Bảo hành đến: ' . Carbon::parse($val->date_end)->format('d/m/Y') . ' - Giá nhập: ' . number_format($val->import_price,0,',','.') . ' đ' . ' - Giá bán: ' . number_format($val->detail_sell_price,0,',','.') . ' đ' . '</li>';
                            } else{
