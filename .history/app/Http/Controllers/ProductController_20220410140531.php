...
class ProductController extends Controller
{
    ...
    public function autocomplete(Request $request)
    {
        if (Auth::check()) {
            ...
            //                 if(Auth::user()->role <= 1){
            //                     $output .= '<li class="li_search_product" data-code="'.$val->product_code.'">' . $val->product_code . ' - ' . $val->brand_name . '  ' . $val->product_name . ' - Bảo hành đến: ' . Carbon::parse($val->date_end)->format('d/m/Y') . ' - Giá nhập: ' . number_format($val->import_price,0,',','.') . ' đ' . ' - Giá bán: ' . number_format($val->sell_price,0,',','.') . ' đ' . '</li>';
            //                 } else{
