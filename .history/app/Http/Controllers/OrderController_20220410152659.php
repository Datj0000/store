        ...
        if (Auth::check()) {
            ...
            if (Session::get('cart')) {
                foreach (Session::get('cart') as $key => $cart) {
                    ...
                    $importdetail = ImportDetail::query()->where('product_code',$details->input('product_code'))->update([
                        'soldout' => $importdetail + $cart['product_quantity'],
                    ]);
                }
                Session::forget('cart');
            }
        ...
        if (Auth::check()) {
            ...
            <table class="table table-separate table-head-custom table-checkable display nowrap" cellspacing="0" width="100%" id="table_cart">
                ...
                foreach (Session::get('cart') as $key => $cart) {
                    ...
                    $import = Import::query()->where('$detail->import_id)->first();
