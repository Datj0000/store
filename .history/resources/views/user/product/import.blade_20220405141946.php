                        ...
                        <div class=" card-body">
                            ...
                            <div class="form-group">
                                ...
                                <input name="fee_ship" type="number" class="form-control form-control-solid" min="0"
                                       id="_fee_ship" placeholder="Phí ship" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==20) return false;"/>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="exampleModalSizeSm5" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel2"
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div id="load_importimportdetail"></div>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    function load_importdetail(id){
        axios.get('fetchdata-importimportdetail/' + id)
            .then(function(response) {
                $("#load_importimportdetail").html(response.data);
            })
            .catch((error) => {
                console.log(error);
            });
        ...
        $(document).on('click', '#create_importdetail', function(e) {
            ...
            validation.validate().then(function(status) {
                if (status == 'Valid') {
                    ...
                    form_data.append("detail_import_price", importdetail_import_price);
                    form_data.append("detail_sell_price", importdetail_sell_price);
                    form_data.append("detail_date_start", importdetail_date_start);
                    form_data.append("detail_date_end", importdetail_date_end);
                    form_data.append("detail_quantity", importdetail_quantity);
                    form_data.append("detail_drive", importdetail_drive);
                    form_data.append("detail_vat", importdetail_vat);
                    axios({
